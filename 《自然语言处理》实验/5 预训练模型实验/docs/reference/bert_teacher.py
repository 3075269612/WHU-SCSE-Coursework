...
def make_data():
    batch = []
    positive = negative = (
        0  # positive若样本中两条样本相邻 加 1 ； 不相邻则negative 加 1 但最终需要保证positive 与negative比例相等
    )
    while positive != batch_size / 2 or negative != batch_size / 2:
        ...
        n_pred = min(
            max_pred, max(1, int(len(input_ids) * 0.15))
        ...
        for pos in cand_maked_pos[:n_pred]:  # 取乱序的索引cand_maked_pos前n_pred 做mask
            ...
            masked_tokens.append(
                input_ids[pos]
            )  # masked_tokens：mask标记对应的原来的token值
            ...
            if random() < 0.8:  # 80% 的 概率 被替换为真正的mask
                ...
                index = randint(
                    0, vocab_size - 1
                )  # random index in vocabulary 从词表中随机选择一个词的索引 可以是本身
                while (
                    index < 4
                ):  # can't involve 'CLS', 'SEP', 'PAD' 但不能是特殊标记，也就是说索引要大于4
                    index = randint(
                        0, vocab_size - 1
                    )  # 索引小于4需要重新获取一个随机数
        ...
        if max_pred > n_pred:
            ...
            masked_tokens.extend(
                [0] * n_pad
            )  # 保证masked_tokens和masked_pos长度始终为max_pred(5)
...
loader = Data.DataLoader(
    MyDataSet(input_ids, segment_ids, masked_tokens, masked_pos, isNext),
    batch_size,
    True,
)
...
"""
max(2):在第2维上取最大值，取完后维度：[batch,max_pred],[batch,max_pred]。此时有两个维度值。第一个是具体的值，第二个是位置
